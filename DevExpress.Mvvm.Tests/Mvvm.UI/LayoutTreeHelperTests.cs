...
#else
            textBlockTemplate = (DataTemplate)XamlReader.Load(
#endif
@"<DataTemplate xmlns=""http://schemas.microsoft.com/winfx/2006/xaml/presentation"" 
xmlns:x=""http://schemas.microsoft.com/winfx/2006/xaml"">
<TextBlock><Run>Text</Run></TextBlock>
...
#else
                Assert.AreSame(Window.Content, LayoutTreeHelper.GetVisualParents(textBox).Where(x => x.GetType() == Window.Content.GetType()).First());
#endif
        ...
        DataTemplate textBlockTemplate;
        [Test, Asynchronous]
        public async Task GetVisualParents2() {
            Grid grid = new Grid();
            ContentControl contentControl = new ContentControl();
            TextBlock textBox;
            TextBlock textBoxContent = null;
            Window.Content = grid;
            grid.Children.Add(contentControl);
            contentControl.ContentTemplate = textBlockTemplate;
            ...
            EnqueueCallback(() => {
                ...
                Assert.AreSame(contentControl, LayoutTreeHelper.GetVisualParents(textBoxContent).Where(x => x is ContentControl).First());
                Assert.AreSame(grid, LayoutTreeHelper.GetVisualParents(textBoxContent).Where(x => x is Grid).First());
                Assert.AreSame(Window.Content, LayoutTreeHelper.GetVisualParents(textBoxContent).Where(x => x.GetType() == Window.Content.GetType()).First());
